...
class CAB(nn.Module):
    ...
    def forward(self, x):
        ...
        res += x
        return res
class Shift_CAB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        super(Shift_CAB, self).__init__()
        modules_body = []
        modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))
        modules_body.append(act)
        modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))
    ...
    def forward(self, x, reverse=False):
        x = self.channel_shift(x, reverse=reverse)
        res = self.body(x)
        res = self.CA(res)
        res += x
        return res
...
class CAB1(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        modules_body.append(conv(n_scale_feat, n_feat, 1, bias=True))
...
class Encoder2(nn.Module):
    def __init__(self, n_features, kernel_size=3, reduction=4, bias=False, scale_unetfeats=48):
        ...
        # n_feats = 48
        self.act = act
        self.encoder_level0 = Shift_CAB(n_feat0, kernel_size, reduction, bias=bias, act=act)
        self.encoder_level0_1 = Shift_CAB(n_feat0, kernel_size, reduction, bias=bias, act=act)
        self.encoder_level1 = Shift_CAB(n_feat, kernel_size, reduction, bias=bias, act=act)
        self.encoder_level1_1 = Shift_CAB(n_feat, kernel_size, reduction, bias=bias, act=act)
    ...
    def forward(self, x, reverse=False):
        ...
        x = self.encoder_level0_1(x, reverse=True)
        x = self.down01(x)
        enc1 = self.encoder_level1(x)
        enc11 = self.encoder_level1_1(enc1, reverse=True)
...
class GShiftNet(nn.Module):
    ...
    def __init__(self, n_features=48, future_frames=0, past_frames=0):
    ...
    def forward(self, x, noise_map=None, k1=None, k2=None, k3=None):
        batch_size, frames, channels, height, width = x.shape
        x = x[0]
        noise_map = noise_map[0]
        shortcut = x
        x0 = self.feat_extract(torch.cat((x,noise_map),dim=1))
        sam_features0, sam_features = self.stage0(x0) 
        decoder_outs = self.stage1(sam_features) 
        output_features = self.stage2(x0[self.num_fb:frames-self.num_ff], sam_features[self.num_fb:frames-self.num_ff], decoder_outs[self.num_fb:frames-self.num_ff])
