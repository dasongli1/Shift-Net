...
class ImageRestorationModel3(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationModel3, self).__init__(opt)
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                if k.startswith('module.stage1.decoder_level'):
    ...
    def feed_data(self, data):
        ...
        # gt_list = [gt[:, i, :, :, :] for i in range(self.n_sequence)]
        # print(gt.shape)
        B, N, C, H, W = gt.shape
        # exit(0)
        
        # self.lq = gt #.to(self.device)#.half()
        r1 = 0.0/255.0
        r2 = 50.0/255.0
        stdn = np.random.rand(1,1,1,1,1) * (r2-r1) + r1
        stdn = torch.FloatTensor(stdn).to(self.device)
        # stdn = torch.empty((1, 1, 1, 1, 1)).uniform_(5/255.0, to=55/255.0)
        noise = torch.zeros_like(gt)
        noise = torch.normal(mean=noise, std=stdn.expand_as(noise))
        lq = noise + gt
        # exit(0)
        self.lq = lq.half()
        noise_map = stdn.expand((B, N, 1, H, W))
        self.noise_map = noise_map.half()
        # print(stdn, noise_map.mean(), noise_map.max(), noise_map.min())

        self.gt = gt[0]# [1:-1]
        # self.gt_1 = gt[0]
        # print(self.lq.shape, self.gt.shape, self.gt_1.shape)
        # exit(0)
        # gt_list = [gt[:, i, :, :, :] for i in range(self.n_sequence)]
        # gt = torch.cat([gt_list[1], gt_list[2], gt_list[3], gt_list[4]], dim=1).to(self.device)
        # self.gt = gt.to(self.device).float()
        # self.gt_1 = torch.cat([gt_list[0], gt_list[1], gt_list[2], gt_list[3], gt_list[4], gt_list[5]], dim=1).to(self.device).float()
        # print(self.lq.shape, self.gt.shape, self.gt_1.shape, self.noise_map.shape)
        # exit(0)
    ...
    def optimize_parameters(self, current_iter):
        ...
        with torch.cuda.amp.autocast():
            output = self.net_g(self.lq, self.noise_map)
        ...
        torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), 0.04)
